module TaskSubmissionFilesDownloadAuthentication
  ...
  TASK_SUBMISSION_FILES_DOWNLOAD_COOKIE_LIFETIME = 2.minutes

  private

  def authenticated_task_submission_files_download_user(unit_id:, task_definition_id:)
    authenticated_task_submission_files_header_user || task_submission_files_download_cookie_user(
      unit_id: unit_id,
      task_definition_id: task_definition_id
    )
  end

  def authenticated_task_submission_files_header_user
  ...
  def task_submission_files_download_cookie_user(unit_id:, task_definition_id:)
    payload = JSON.parse(cookies.encrypted[TASK_SUBMISSION_FILES_DOWNLOAD_COOKIE].to_s)
